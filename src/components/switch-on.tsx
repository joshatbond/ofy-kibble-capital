 ...
 */
export function Case(_props: { predicate: boolean; children: ReactNode }) {
  return null
}

/**
 * Ordered exclusive render: mounts the `children` of the first {@link Case} whose
 * `predicate` is `true`. Returns `null` when no case matches. Non-`Case` children
 * are skipped.
 */
export function SwitchOn(props: { children: ReactNode }) {
  for (const child of Children.toArray(props.children)) {
    if (!isCaseElement(child)) continue
    if (child.props.predicate) return child.props.children
  }
  return null
}
...
function isCaseElement(
  child: ReactNode
): child is ReactElement<{ predicate: boolean; children: ReactNode }> {
